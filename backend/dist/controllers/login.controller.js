...
const login = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!user) {
            res.status(404).json({ msg: 'Nombre de usuario incorrecto' });
            return;
        }
        // Verificar si el usuario está activo
        if (user.get('estado') !== 'activo') {
            res.status(403).json({ msg: 'El usuario no está activo' });
            return;
        }
        ...
        const token = jsonwebtoken_1.default.sign({ usuario: usuario }, 'secreto', { expiresIn: '30m' }); // Cambia 'secreto' por tu clave secreta
